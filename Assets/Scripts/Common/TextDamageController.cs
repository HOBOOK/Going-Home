...
public class TextDamageController : MonoBehaviour {
    ...
    void DamagePop()
    {
        if(isCritical)
        {
            ...
            Vector2 popVelocity = status.isLeftorRight ? new Vector2(Random.Range(-3, -6), Random.Range(3,6)) : new Vector2(Random.Range(3, 6), Random.Range(3, 6));
        ...
        else
        {
            this.GetComponent<Rigidbody2D>().constraints = RigidbodyConstraints2D.None;
            textMesh.SetActive(true);
            Vector2 popVelocity = status.isLeftorRight ? new Vector2(Random.Range(-2, -4), Random.Range(2, 4)) : new Vector2(Random.Range(2, 4), Random.Range(2, 4));
            GetComponent<Rigidbody2D>().AddForce(popVelocity, ForceMode2D.Impulse);
